...
module.exports = function (_Reporter) {
  ...
  function ESLintAngularReporter(options, projectName) {
    ...
    var _this = _possibleConstructorReturn(this, (ESLintAngularReporter.__proto__ || Object.getPrototypeOf(ESLintAngularReporter)).call(this, options, projectName));
    ...
    value: function processFile(file) {
      ...
      try {
        for (var _iterator = result.results[0].messages[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          this.addIssue(message.line ? message.line : null, message.message, '', 'ng_' + message.ruleId.replace('angular/', '').replace(/-/g, '_'), severity, ReporterType.ESLINTANGULAR);
        }
      } catch (err) {
