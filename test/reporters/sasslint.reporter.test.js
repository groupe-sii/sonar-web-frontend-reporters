...
const fs = require('fs'),
  ...
  SASSLintReporterES5 = require('../../build/reporters/sasslint.reporter'),
  ...
  describe('SASSLintReporter', () => {
    ...
    describe('#launch', () => {
      ...
      it('should have 8 issues', (done) => {
        let reporter = new SASSLintReporter(sassLintMock.defaultOptions, 'SonarWebFrontEndReporters'),
          expected = ['no-color-literals', 'hex-notation', 'property-sort-order', 'no-misspelled-properties', 'space-before-brace', 'no-ids', 'no-css-comments'];
      ...
      it ('shouldn\'t match the ignored file', (done) => {
        let reporter = new SASSLintReporter(sassLintMock.multiSrcOption, 'SonarWebFrontEndReporters');

        reporter.launch(() => {
          let result = readJSONFile(sassLintMock.multiSrcOption.report);
